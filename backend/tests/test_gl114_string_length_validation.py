...
class _BaseGl114(unittest.TestCase):
    ...
    def _run_raw_handler(self, handler):
        ...
        if isinstance(candidate_body, dict):
            ...
            if path.startswith("/v1/grants/") and path.endswith("/revoke"):
                return 200, {"ok": True}
            if "/v1/grant-requests/" in path and path.endswith("/deny"):
                return 200, {"request": {"denial_reason": candidate_body.get("reason")}}
            if path == "/v1/grants" and candidate_body.get("role") != "operator":
...
class TestGl114ValidationHelper(_BaseGl114):
    ...
    def test_validate_string_length_accepts_normal_string(self):
        from backend.src.core.validation import validate_string_length
    ...
    def test_validate_string_length_rejects_overlong(self):
        from backend.src.core.validation import validate_string_length, ValidationError
    ...
    def test_validate_optional_string_length_accepts_none(self):
        from backend.src.core.validation import validate_optional_string_length
    ...
    def test_validate_optional_string_length_rejects_overlong(self):
        from backend.src.core.validation import validate_optional_string_length, ValidationError
        with self.assertRaises(ValidationError):
...
class TestGl114ServerApiBoundaryValidation(_BaseGl114):
    ...
    def test_post_grant_rejects_overlong_subject_id(self):
        ...
        handler = self._make_raw_handler("/v1/grants", method="POST", auth_header="Bearer owner-token", body=body)
    ...
    def test_post_challenge_rejects_overlong_subject_id(self):
        ...
        body = json.dumps(payload).encode()
        handler = self._make_raw_handler("/v1/challenges", method="POST", auth_header="Bearer owner-token", body=body)
        status, data = self._run_raw_handler(handler)
        self.assertEqual(status, 400)
        self.assertEqual(data["errorCode"], "invalid_field")
        self.assertIn("subjectId", data["reason"])
    ...
    def test_post_grant_request_rejects_overlong_reason(self):
        ...
        body = json.dumps(payload).encode()
        handler = self._make_raw_handler("/v1/grant-requests", method="POST", auth_header="Bearer owner-token", body=body)
        status, data = self._run_raw_handler(handler)
        self.assertEqual(status, 400)
        self.assertEqual(data["errorCode"], "invalid_field")
        self.assertIn("reason", data["reason"])
    ...
    def test_post_grant_request_deny_rejects_overlong_reason(self):
        ...
        handler = self._make_raw_handler(
            f"/v1/grant-requests/{created.id}/deny",
            method="POST",
