"""Tests for GL-070 Persistence Backend Boundary and PostgreSQL Readiness Design.

Lightweight validation test proving the persistence backend and PostgreSQL readiness design is:
...
- explicitly defining persistence boundaries, SQLite constraints, PostgreSQL readiness,
  migration expectations, transaction semantics, and production fail-closed rules
...
class TestGL070PersistenceBackendPostgreSQLReadiness(unittest.TestCase):
    """GL-070: Validate persistence backend boundary and PostgreSQL readiness design."""

    REPO_ROOT = pathlib.Path(__file__).with_suffix("").parent.parent.parent
    DOCS_DIR = REPO_ROOT / "docs"
    EXAMPLE_DIR_GL070 = DOCS_DIR / "examples" / "gl070"
    BACKEND_SRC_DIR = REPO_ROOT / "backend" / "src"

    REQUIRED_PERSISTENCE_CONTRACTS = [
        "stable_persistent_ids",
        "grant_request_records",
        "grant_records",
        "grant_execution_records",
        "evidence_bundle_records",
        "evidence_verification_records",
        "provenance_audit_records",
        "policy_compliance_records",
        "operator_access_records",
        "transaction_expectations",
        "deterministic_test_fixtures",
    ]

    REQUIRED_BACKEND_BOUNDARY_RULES = [
        "Product Core should not depend on SQLite-specific behavior",
        "Backend-specific connection details should stay behind configuration boundaries",
        "Database credentials are secret-bearing config and must use approved secret sources",
        "Storage backend choice should not change public API contracts",
        "Persistence errors should map into stable error semantics",
        "Production mode should fail closed if persistence config is missing or invalid",
    ]
    ...
    REQUIRED_READINESS_CATEGORIES = [
        ...
        "production-fail-closed-persistence",
    ]
    ...
    @classmethod
    def setUpClass(cls):
        cls.doc_path = cls.DOCS_DIR / "persistence_backend_postgresql_readiness_design.md"
        cls.matrix_path = cls.EXAMPLE_DIR_GL070 / "persistence_backend_matrix.json"
        cls.catalog_path = cls.EXAMPLE_DIR_GL070 / "postgresql_readiness_catalog.json"
    ...
    # ── 1. Persistence backend doc exists ─────────────────────────────
    def test_persistence_backend_doc_exists(self):
        self.assertTrue(
            self.doc_path.exists(),
            "docs/persistence_backend_postgresql_readiness_design.md must exist",
        )

    # ── 2. Persistence backend matrix JSON exists and parses ──────────
    def test_matrix_exists_and_parses(self):
        self.assertTrue(
            self.matrix_path.exists(),
            "docs/examples/gl070/persistence_backend_matrix.json must exist",
        )
        self.assertIsNotNone(self.matrix_json, "Matrix must parse as valid JSON")
        self.assertIsInstance(self.matrix_json, dict)

    # ── 3. PostgreSQL readiness catalog JSON exists and parses ────────
    def test_catalog_exists_and_parses(self):
        self.assertTrue(
            self.catalog_path.exists(),
            "docs/examples/gl070/postgresql_readiness_catalog.json must exist",
        )
    ...
    # ── 5. GL-069 deployment package runtime modes doc exists ────────
    def test_gl069_deployment_package_doc_exists(self):
        path = self.DOCS_DIR / "deployment_package_runtime_modes_design.md"
        self.assertTrue(path.exists(), "GL-069 deployment package runtime modes doc must exist")

    # ── 6. GL-068 secret management baseline doc exists ──────────────
    ...
    # ── 8. Persistence backend matrix includes SQLite and PostgreSQL ──
    def test_matrix_includes_sqlite_and_postgresql(self):
        backends = self.matrix_json.get("persistenceBackends", [])
        backend_names = {b.get("backend") for b in backends}
        self.assertIn("sqlite", backend_names, "Matrix must include sqlite backend")
        self.assertIn("postgresql", backend_names, "Matrix must include postgresql backend")

    # ── 9. SQLite is marked non-production or not production-sufficient
    def test_sqlite_marked_non_production(self):
        backends = self.matrix_json.get("persistenceBackends", [])
        sqlite_entry = None
        for b in backends:
            if b.get("backend") == "sqlite":
                sqlite_entry = b
                break
        self.assertIsNotNone(sqlite_entry, "SQLite entry must exist")
        self.assertFalse(sqlite_entry.get("productionSuitable", True), "SQLite must not be marked production-suitable")

    # ── 10. PostgreSQL is marked as production candidate but not implemented/ready
    def test_postgresql_marked_production_candidate_not_ready(self):
        backends = self.matrix_json.get("persistenceBackends", [])
        pg_entry = None
        for b in backends:
            if b.get("backend") == "postgresql":
                pg_entry = b
                break
        self.assertIsNotNone(pg_entry, "PostgreSQL entry must exist")
        self.assertFalse(pg_entry.get("productionSuitable", True), "PostgreSQL must not yet be marked production-suitable")
        self.assertEqual(pg_entry.get("implementationStatus"), "not-implemented", "PostgreSQL must be not-implemented")

    # ── 11. PostgreSQL readiness catalog includes required readiness categories
    def test_catalog_includes_required_readiness_categories(self):
        categories = self.catalog_json.get("readinessCategories", [])
        category_ids = {c.get("id") for c in categories}
        for expected in self.REQUIRED_READINESS_CATEGORIES:
            with self.subTest(category=expected):
                self.assertIn(
                    expected,
                    category_ids,
                    f"Catalog must include readiness category '{expected}'",
                )

    # ── 12. Production claim rules state production persistence readiness is not yet claimed
    def test_production_claim_rules_state_not_claimed(self):
        rules = self.matrix_json.get("productionClaimRules", [])
        rules_text = " ".join(rules).lower()
        self.assertIn(
            "not yet claimed",
            rules_text,
            "Matrix productionClaimRules must state production persistence readiness is not yet claimed",
        )

    # ── 13. Document states SQLite local/test/demo convenience must not become production behavior
    def test_doc_states_sqlite_not_production_behavior(self):
        content = self.doc_path.read_text(encoding="utf-8").lower()
        self.assertIn(
            "local, test, and demo sqlite convenience must not accidentally become production persistence behavior",
            content,
        )
        self.assertIn(
            "not sufficient alone for production-readiness claims",
            content,
        )

    # ── 14. Document states production persistence readiness is not yet claimed
    def test_doc_states_production_persistence_readiness_not_claimed(self):
        content = self.doc_path.read_text(encoding="utf-8").lower()
        self.assertIn(
        ...
        self.assertIn(
            "database backend selection, postgresql compatibility, migrations, transactions, secrets, ci, backup/restore, and operational runbooks",
            content,
        )

    # ── 15. JSON files do not contain obvious secrets, fake tokens, fake DB URLs, fake keys, fake credentials
    def _sanitize_known_safe_terms(self, text):
        ...
        safe_terms = [
            ...
            "grant creation transaction",
            "evidence persistence transaction",
            ...
            "persistence startup check",
            "persistence startup checks",
            "startup validation",
            ...
            "not yet claimed",
            "not production-ready",
            "become production deployment behavior",
            "become production behavior",
            "become production persistence behavior",
            "production persistence readiness",
            "claim production persistence readiness",
            "not claim production persistence readiness",
            "grantlayer",
        ...
        lower = text.lower()
        safe_terms_sorted = sorted(safe_terms, key=lambda t: len(t), reverse=True)
        for term in safe_terms_sorted:
            lower = lower.replace(term, "")
        return lower

    def test_json_no_real_secrets(self):
        """JSON files should not contain fake tokens/pwds/keys."""
        for name, text in [
            ("matrix", self.matrix_text),
            ("catalog", self.catalog_text),
        ]:
            sanitized = self._sanitize_known_safe_terms(text)
            for pattern in self.SECRET_PATTERNS:
                self.assertNotIn(
                    pattern,
                    sanitized,
                    f"{name} JSON appears to contain a secret-like value after safe-term sanitization: '{pattern}'",
                )

    # ── 16. Forbidden implementation files were not introduced ────────
    def test_no_forbidden_files_created(self):
