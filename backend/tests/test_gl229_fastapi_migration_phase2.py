...
class TestAuditEventsEndpoint(_GL229TestBase):
    ...
    def test_audit_events_invalid_limit_422(self):
        resp = self.client.get("/v1/audit-events?limit=0")
...
class TestGrantRequestsOperatorModelDisabled(_GL229TestBase):
    ...
    def test_create_grant_request_requires_operator_model(self):
        resp = self.client.post("/v1/grant-requests", json=self.BODY)
        self.assertEqual(resp.status_code, 404)
        self.assertIn("operator_model_disabled", str(resp.json()))

    def test_approve_grant_request_requires_operator_model(self):
        resp = self.client.post("/v1/grant-requests/fake-id/approve")
        self.assertEqual(resp.status_code, 404)

    def test_deny_grant_request_requires_operator_model(self):
        resp = self.client.post("/v1/grant-requests/fake-id/deny", json={"reason": "test"})
        self.assertEqual(resp.status_code, 404)
    ...
    def test_list_grant_requests_status_filter_invalid(self):
        resp = self.client.get("/v1/grant-requests?status=invalid_status_xyz")
...
class TestExecutionsOperatorModelDisabled(_GL229TestBase):
    ...
    def test_get_execution_requires_operator_model(self):
        resp = self.client.get("/v1/grant-executions/some-id")
        self.assertEqual(resp.status_code, 404)

    def test_list_executions_for_grant_requires_operator_model(self):
        resp = self.client.get("/v1/grants/some-id/executions")
        self.assertEqual(resp.status_code, 404)
...
class TestEvidenceEndpoints(_GL229TestBase):
    ...
    def test_evidence_bundle_404_unknown_execution(self):
        resp = self.client.get("/v1/evidence/executions/nonexistent-exec")
        self.assertEqual(resp.status_code, 404)
        detail = resp.json()
        self.assertIn("execution_not_found", str(detail))

    def test_evidence_export_404_unknown_execution(self):
        resp = self.client.get("/v1/evidence/executions/nonexistent-exec/export")
        self.assertEqual(resp.status_code, 404)

    def test_evidence_verify_404_unknown_execution(self):
        resp = self.client.get("/v1/evidence/executions/nonexistent-exec/verify")
        self.assertEqual(resp.status_code, 404)

    def test_evidence_completeness_404_unknown_execution(self):
        resp = self.client.get("/v1/evidence/executions/nonexistent-exec/completeness")
        self.assertEqual(resp.status_code, 404)
...
class TestProvenanceEndpoints(_GL229TestBase):
    ...
    def test_provenance_summary_404_unknown(self):
        resp = self.client.get("/v1/provenance/executions/nonexistent-exec/summary")
...
class TestAuditorEndpoints(_GL229TestBase):
    ...
    def test_auditor_report_404_unknown_execution(self):
        resp = self.client.get("/v1/auditor/reports/executions/nonexistent-exec")
        self.assertEqual(resp.status_code, 404)
        self.assertIn("execution_not_found", str(resp.json()))

    def test_auditor_export_build_200(self):
        resp = self.client.post("/v1/auditor/exports/build", json={})
        self.assertEqual(resp.status_code, 200)
    ...
    def test_auditor_export_build_with_data(self):
        ...
        resp = self.client.post("/v1/auditor/exports/build", json=body)
        self.assertEqual(resp.status_code, 200)
...
class TestComplianceEndpoints(_GL229TestBase):
    ...
    def test_compliance_gaps_404_unknown(self):
        resp = self.client.get("/v1/compliance/gaps/executions/nonexistent-exec")
        self.assertEqual(resp.status_code, 404)
        self.assertIn("execution_not_found", str(resp.json()))

    def test_compliance_readiness_build_200(self):
        resp = self.client.post("/v1/compliance/readiness/build", json={})
        self.assertEqual(resp.status_code, 200)
        data = resp.json()
    ...
    def test_compliance_readiness_build_with_subject(self):
        resp = self.client.post("/v1/compliance/readiness/build", json={"subjectId": "agent-001"})
        self.assertEqual(resp.status_code, 200)
...
class TestAdminOperatorsNoToken(_GL229TestBase):
    ...
    def test_get_operator_no_token_403(self):
        resp = self.client.get("/v1/admin/operators/some-id")
        self.assertIn(resp.status_code, (401, 403))

    def test_create_operator_no_token_403(self):
        body = {"name": "test", "role": "owner", "tenantId": "t1"}
        resp = self.client.post("/v1/admin/operators", json=body)
        self.assertIn(resp.status_code, (401, 403))

    def test_revoke_operator_no_token_403(self):
        resp = self.client.post("/v1/admin/operators/some-id/revoke")
        self.assertIn(resp.status_code, (401, 403))
...
class TestAdminOperatorsWithToken(_GL229TestBase):
    ...
    def test_list_operators_200(self):
        resp = self.client.get("/v1/admin/operators", headers=self._admin_headers())
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), list)

    def test_create_operator_201(self):
        body = {"name": "GL229 TestOp", "role": "grant_admin", "tenantId": "tenant-gl229"}
        resp = self.client.post("/v1/admin/operators", json=body, headers=self._admin_headers())
    ...
    def test_get_operator_by_id(self):
        ...
        create_resp = self.client.post("/v1/admin/operators", json=body, headers=self._admin_headers())
        self.assertEqual(create_resp.status_code, 201)
        op_id = create_resp.json()["operatorId"]
        get_resp = self.client.get(f"/v1/admin/operators/{op_id}", headers=self._admin_headers())
    ...
    def test_get_nonexistent_operator_404(self):
        resp = self.client.get("/v1/admin/operators/does-not-exist-xyz", headers=self._admin_headers())
        self.assertEqual(resp.status_code, 404)
    ...
    def test_revoke_operator(self):
        ...
        create_resp = self.client.post("/v1/admin/operators", json=body, headers=self._admin_headers())
        op_id = create_resp.json()["operatorId"]
        revoke_resp = self.client.post(f"/v1/admin/operators/{op_id}/revoke", headers=self._admin_headers())
    ...
    def test_revoke_nonexistent_operator_404(self):
        resp = self.client.post("/v1/admin/operators/ghost-id-xyz/revoke", headers=self._admin_headers())
        self.assertEqual(resp.status_code, 404)
...
class TestChallengesEndpoints(_GL229TestBase):
    ...
    def test_create_challenge_201(self):
        ...
        resp = self.client.post("/v1/challenges", json=body)
        self.assertEqual(resp.status_code, 201)
        data = resp.json()
...
class TestAgentPermissionsEndpoints(_GL229TestBase):
    ...
    def test_list_profiles_200(self):
        resp = self.client.get("/v1/agent-permissions/profiles")
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), list)

    def test_get_profile_404_unknown(self):
        resp = self.client.get("/v1/agent-permissions/profiles/does-not-exist-xyz")
        self.assertEqual(resp.status_code, 404)
    ...
    def test_evaluate_permission_200(self):
        ...
        resp = self.client.post("/v1/agent-permissions/evaluate", json=body)
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), dict)
    ...
    def test_resolve_assignment_200(self):
        ...
        resp = self.client.post("/v1/agent-permissions/assignments/resolve", json=body)
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), dict)
...
class TestApprovalsEndpoints(_GL229TestBase):
    ...
    def test_lifecycle_build_200(self):
        resp = self.client.post("/v1/approvals/lifecycle/build", json={})
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), dict)

    def test_lifecycle_transition_missing_fields_400(self):
        resp = self.client.post("/v1/approvals/lifecycle/transition", json={})
        self.assertEqual(resp.status_code, 400)
        self.assertIn("missing_required_fields", str(resp.json()))
    ...
    def test_lifecycle_transition_200(self):
        ...
        resp = self.client.post("/v1/approvals/lifecycle/transition", json=body)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_evaluate_approvals_200(self):
        ...
        resp = self.client.post("/v1/approvals/evaluate", json=body)
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), dict)
...
class TestDecisionProvenanceEndpoints(_GL229TestBase):
    ...
    def test_build_provenance_v2_200(self):
        resp = self.client.post("/v1/decision-provenance/v2/build", json={})
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), dict)
    ...
    def test_build_provenance_v2_with_data(self):
        ...
        resp = self.client.post("/v1/decision-provenance/v2/build", json=body)
        self.assertEqual(resp.status_code, 200)
...
class TestPolicyRequirementsEndpoints(_GL229TestBase):
    ...
    def test_evaluate_policy_200(self):
        resp = self.client.post("/v1/policy-requirements/evaluate", json={})
        self.assertEqual(resp.status_code, 200)
        self.assertIsInstance(resp.json(), dict)

    def test_evaluate_policy_with_data(self):
        body = {"policyPack": "standard-v1", "subject": {"id": "agent-001"}}
        resp = self.client.post("/v1/policy-requirements/evaluate", json=body)
        self.assertEqual(resp.status_code, 200)
...
class TestRouteCount(_GL229TestBase):
    ...
    def test_all_phase2_routes_registered(self):
        ...
        expected = {
            "/v1/audit-events",
            "/v1/grant-requests",
            "/v1/grant-requests/{request_id}",
            "/v1/grant-requests/{request_id}/approve",
            "/v1/grant-requests/{request_id}/deny",
            "/v1/grant-executions",
            "/v1/grant-executions/{execution_id}",
            "/v1/grants/{grant_id}/executions",
            "/v1/evidence/executions/{execution_id}",
            "/v1/evidence/executions/{execution_id}/export",
            "/v1/evidence/executions/{execution_id}/verify",
            "/v1/evidence/executions/{execution_id}/completeness",
            "/v1/provenance/executions/{execution_id}/summary",
            "/v1/auditor/reports/executions/{execution_id}",
            "/v1/auditor/exports/build",
            "/v1/compliance/gaps/executions/{execution_id}",
            ...
            "/v1/agent-permissions/profiles/{profile_name}",
