"""GL-094C: Production Architecture and Operations Readiness Review — artifact validation tests.
...
MD_PATH = os.path.join(
    REPO_ROOT, "docs", "production_architecture_operations_readiness_review.md"
)
JSON_PATH = os.path.join(
    ...
    "examples",
    "gl094c",
    "production_architecture_operations_findings.json",
)
...
VALID_STATUSES = {"confirmed", "suspected", "test_gap", "design_gap", "resolved_by_existing_work"}
...
class TestGl094cMarkdownArtifact(unittest.TestCase):
    def _content(self):
    ...
    def test_markdown_contains_production_readiness_blockers(self):
        self.assertIn("Production readiness blockers", self._content())

    def test_markdown_contains_staging_readiness_assessment(self):
        self.assertIn("Staging readiness assessment", self._content())
...
class TestGl094cJsonArtifact(unittest.TestCase):
    def _data(self):
    ...
    def test_review_id(self):
        self.assertEqual(self._data()["reviewId"], "GL-094C")

    def test_review_type(self):
        self.assertEqual(
            self._data()["reviewType"],
            "production_architecture_operations_readiness_review",
        )

    def test_staging_readiness_exists(self):
        data = self._data()
        self.assertIn("stagingReadiness", data)
        self.assertIsInstance(data["stagingReadiness"], str)
        self.assertTrue(len(data["stagingReadiness"]) > 0)

    def test_production_readiness_exists(self):
        data = self._data()
        self.assertIn("productionReadiness", data)
        self.assertIsInstance(data["productionReadiness"], str)
        self.assertTrue(len(data["productionReadiness"]) > 0)
...
class TestGl094cNoBranchProductionChanges(unittest.TestCase):
    ...
    def test_expected_artifact_files_changed(self):
        expected = {
            ...
            "docs/examples/gl094c/production_architecture_operations_findings.json",
            "backend/tests/test_gl094c_production_architecture_operations_review_artifact.py",
        }
    ...
    def test_only_allowed_files_changed(self):
        if self._current_branch() != "gl-094c-production-architecture-operations-review":
            self.skipTest(
                "Allowed-files diff check only valid on original GL-094C feature branch"
            )
        changed = self._changed_files()
        allowed = {
            "docs/production_architecture_operations_readiness_review.md",
