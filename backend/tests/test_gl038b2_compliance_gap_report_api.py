"""GL-038-B2 — Compliance Gap Report API Endpoint tests.

Covers endpoint routing, auth, response shape, 404 handling,
includeDetails flag, overall status logic, and secrets safety.
"""
...
class TestComplianceGapReportAPI(unittest.TestCase):
    ...
    def test_endpoint_exists_and_returns_404_for_unknown_execution(self):
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/nonexistent-id",
    ...
    def test_endpoint_returns_200_for_existing_execution(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-api-1",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        self.assertEqual(body["reportType"], "compliance_gap_report")
    ...
    def test_endpoint_requires_admin_token_when_operator_disabled(self):
        ...
        status, body = self._run_handler("/v1/compliance/gaps/executions/ex-auth-1")
        self.assertEqual(status, 401)
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-auth-1",
    ...
    def test_endpoint_accepts_operator_roles_when_operator_enabled(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-auth-2",
    ...
    def test_endpoint_rejects_demo_operator_role(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-auth-3",
    ...
    def test_response_contains_minimum_fields(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-shape-1",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        required_fields = {
        ...
        self.assertEqual(body["reportType"], "compliance_gap_report")
        self.assertEqual(body["reportVersion"], "gl-compliance-gap-v1")

    def test_response_overall_status_clear_when_no_gaps(self):
        self._make_execution("ex-clear", grant_id="g-clear")
        grant = self.Grant(
            id="g-clear",
            subject_id="sub-1",
            role="tech",
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-clear",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        self.assertEqual(body["overallStatus"], "clear")
        self.assertEqual(body["severity"], "none")
        self.assertEqual(body["complianceGaps"], [])
        self.assertEqual(body["recommendedActions"], ["no_action_required"])

    def test_response_overall_status_gaps_detected_for_non_critical(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-partial",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        self.assertEqual(body["overallStatus"], "gaps_detected")
        self.assertIn("missing_provenance_events", [g["gapId"] for g in body["complianceGaps"]])

    def test_response_overall_status_blocked_for_critical_gap(self):
        grant = self.Grant(
            id="g-blocked",
            subject_id="sub-1",
            role="tech",
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-blocked",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        self.assertEqual(body["overallStatus"], "blocked")
        self.assertEqual(body["severity"], "critical")
        gap_ids = [g["gapId"] for g in body["complianceGaps"]]
    ...
    def test_include_details_true_includes_provenance_events(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-det-on?includeDetails=true",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
    ...
    def test_include_details_false_omits_checks(self):
        ...
        self.assertIn("overallStatus", body)
        self.assertIn("severity", body)
        self.assertIn("complianceGaps", body)
        self.assertIn("blockingGaps", body)
        self.assertIn("recommendedActions", body)
    ...
    def test_response_does_not_expose_secrets(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-sec-1",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        raw = json.dumps(body)
    ...
    def test_no_bundle_json_in_response(self):
        ...
        status, body = self._run_handler(
            "/v1/compliance/gaps/executions/ex-nobundle",
            auth="Bearer test-admin-token"
        )
        self.assertEqual(status, 200)
        result_str = str(body)
